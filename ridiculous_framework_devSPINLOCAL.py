...
from rotation_matching import rot_storm_matcher

def multi_case_algorithm_ML1_devSPINLOCAL(storm_relative_dir, zdrlev, kdplev, REFlev, REFlev1, big_storm, zero_z_trigger, storm_to_track, year, month, day, hour, start_min, duration, calibration, station, h_Z0C, Bunkers_m, localfolder, track_dis=10, GR_mins=5.0):
...
#     results = conn.download(scans, 'RadarFolder')
    ...
    g = Geod(ellps='sphere')
    #Open the placefile
    f = open("SPORKSPIN"+station+str(dt.year)+str(dt.month)+str(dt.day)+str(dt.hour)+str(dt.minute)+"_Placefile.txt", "w+")
    ...
    #Local file option:
        ...
        [Zint,REF,KDP,CC,CC_c,CCall,ZDRmasked1,ZDRrmasked1,REFmasked,REFrmasked,KDPmasked,KDPrmasked,rlons,rlats,rlons_2d,rlats_2d,cenlat,cenlon,VEL, REFall] = gridding_spin(radar,radar_v,Z0C)
        ...
        if len(max_lons_c) > 0:
            ...
            [rot_mag1, rot_lat1, rot_lon1, rot_storm_lon_1, rot_storm_lat_1] = rot_storm_matcher(shear_maxes1,shear_lats1, shear_lons1,ax,f,time_start,month,d_beg,h_beg,min_beg,sec_beg,d_end,h_end,min_end,sec_end,rlons,rlats,max_lons_c,max_lats_c,proj)
            
            #Getting 1.5km rotation objects
            [rot_mag15, rot_lat15, rot_lon15, rot_storm_lon_15, rot_storm_lat_15] = rot_storm_matcher(shear_maxes15,shear_lats15, shear_lons15,ax,f,time_start,month,d_beg,h_beg,min_beg,sec_beg,d_end,h_end,min_end,sec_end,rlons,rlats,max_lons_c,max_lats_c,proj)
            
            #Getting 2km rotation objects
            [rot_mag2, rot_lat2, rot_lon2, rot_storm_lon_2, rot_storm_lat_2] = rot_storm_matcher(shear_maxes2,shear_lats2, shear_lons2,ax,f,time_start,month,d_beg,h_beg,min_beg,sec_beg,d_end,h_end,min_end,sec_end,rlons,rlats,max_lons_c,max_lats_c,proj)
            
            #Getting 2.5km rotation objects
            [rot_mag25, rot_lat25, rot_lon25, rot_storm_lon_25, rot_storm_lat_25] = rot_storm_matcher(shear_maxes25,shear_lats25, shear_lons25,ax,f,time_start,month,d_beg,h_beg,min_beg,sec_beg,d_end,h_end,min_end,sec_end,rlons,rlats,max_lons_c,max_lats_c,proj)
...
#             zdr_con_areas1 = zdr_con_areas_arr[zdr_con_areas_arr > area]
            ...
            if np.asarray(rot1_con_lon).shape[0] > 0:
                ax.scatter(rot1_con_lon, rot1_con_lat, marker = 'v', s = 100, color = 'red', zorder = 20, transform=ccrs.PlateCarree())
            if np.asarray(rot15_con_lon).shape[0] > 0:
                ax.scatter(rot15_con_lon, rot15_con_lat, marker = 'v', s = 100, color = 'orange', zorder = 20, transform=ccrs.PlateCarree())
            if np.asarray(rot2_con_lon).shape[0] > 0:
                ax.scatter(rot2_con_lon, rot2_con_lat, marker = 'v', s = 100, color = 'yellow', zorder = 20, transform=ccrs.PlateCarree())
            if np.asarray(rot25_con_lon).shape[0] > 0:
                ax.scatter(rot25_con_lon, rot25_con_lat, marker = 'v', s = 100, color = 'green', zorder = 20, transform=ccrs.PlateCarree())
        ...
        alt_levs = [1000, 2000]
        plt.savefig('Machine_Learning/SPORK_SPIN'+station+str(time_start.year)+str(time_start.month)+str(day)+str(hour)+str(minute)+'.png')
        print('Figure Saved')
        print(p_rot1)
        print(p_rot15)
        print(p_rot2)
